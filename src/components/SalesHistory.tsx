...
export function SalesHistory() {
  const { sales, shop, userName, userEmail, refreshData, products, returnSale } = useApp(); // ✅ added returnSale
  ...
  // Helper to get product image by productId (or productName fallback)
  const getProductImage = (productId?: string, productName?: string) => {
    if (productId) {
      const product = products.find(p => p.id === productId);
      if (product && (product as any).imageUrl) return (product as any).imageUrl;
    }
    // Fallback: try to match by name (less reliable but keeps images showing)
    if (productName) {
      const matched = products.find(p => p.name === productName);
      if (matched && (matched as any).imageUrl) return (matched as any).imageUrl;
    }
    return null;
  };
  ...
  const handleReturn = async (saleId: string) => {
    if (!window.confirm("Are you sure you want to return this entire order? Stock will be rolled back.")) return;
    try {
      await returnSale(saleId);
      toast.success("Sale returned successfully!");
      setSelectedSale(null);
    } catch (error: any) {
      toast.error(error.message || "Failed to return sale");
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredSales.map((sale) => (
                <tr key={sale.id} className="hover:bg-green-50/30 transition-colors">
                  ...
                  <td className="p-3">
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${sale.status === 'returned' ? 'bg-red-100 text-red-700 border border-red-200' : 'bg-green-100 text-green-700 border border-green-200'}`}>
        ...
        {/* Details Sidebar - ✅ Now shows product images */}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {(sales as any[]).find(s => s.id === selectedSale)?.items.map((item: any, i: number) => {
                  const productImg = getProductImage(item.productId, item.productName);
                  return (
                    <div key={i} className="flex items-center gap-2 border-b border-gray-50 pb-2">
                      {productImg ? (
                        <img src={productImg} alt={item.productName} className="w-8 h-8 object-cover rounded-md border border-gray-200" />
                      ) : (
                        <div className="w-8 h-8 bg-gray-100 rounded-md flex items-center justify-center text-gray-400 text-xs">
                          📷
                        </div>
                      )}
                      <div className="flex-1">
                        <div className="flex justify-between text-[11px]">
                          ...
                          <span className="font-bold text-gray-800">RS. {item.total}</span>
                        </div>
                        <div className="text-[10px] text-gray-400">Qty: {item.quantity} × RS. {item.price}</div>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="pt-2">
                ...
                <div className="flex gap-2 mt-4">
                  <button 
                    ...
                    className="flex-1 bg-gray-900 text-white py-2 rounded-md font-bold text-xs hover:bg-black transition-all flex items-center justify-center gap-2"
                  >
                    <Printer size={14} /> Print
                  </button>
                  
                  {((sales as any[]).find(s => s.id === selectedSale))?.status !== 'returned' && (
                    <button 
                      onClick={() => handleReturn(selectedSale)}
                      className="flex-1 bg-red-50 text-red-600 border border-red-200 py-2 rounded-md font-bold text-xs hover:bg-red-100 transition-all flex items-center justify-center gap-2"
                    >
